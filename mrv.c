...
CM_STAT cmp_proc_022 (
  ...
  for (occ=0;marc_get_field(pp->inmp,22,occ,&srcp,&src_len)==0;occ++) {
    ...
    /*=======================================================================*/
    /* If no $9, exclude $0 and output everything else as-is...              */
    /*=======================================================================*/
    if(rc!=0) {
      /*=====================================================================*/
	...
	if (sf_code != '0')
      ...
      /*=====================================================================*/
      /* If $9 = 'Y', exclude $0 & $9 and output everything else as-is...    */
      /*=====================================================================*/
      if((*tmpp=='y') || (*tmpp=='Y')) {
...
CM_STAT cmp_proc_035 (
  ...
  /* Extract Leader 7... */
  cmp_buf_find(pp, "999:0$a:0", &srcp, &src_len);
  auth999a = ((strncmp((char *)srcp,"AUTH",4)==0)&&(src_len==4));
...
/*===========================================================================*/
/* LEADER 07 IS NOT 's' OR 'b'                                               */
/*===========================================================================*/

    /* Leader 07 not 's' or 'b' */
	...
	if (sf_code == 'a') {
	  ...
	  if ((strncmp((char *)srcp,"(DNLM)",6) == 0) &&
	    ...
	    cm_error(CM_WARNING,"LDR 07 not 's' or 'b' but 035$a='(DNLM)...(s)'");
	  }

	  else {
	    /* Output first $a starting with "(OCoLC)", */
...
/*===========================================================================*/
/* LEADER 07 IS 's' OR 'b' -- FIRST PASS                                     */
/*===========================================================================*/
...
/*===========================================================================*/
	...
	for (socc=2;
	  ...
	  /* Ignore any $a's that start with "(DNLM)" */
	  /* but do not end with "(s)" */
	  if (sf_code == 'a') {
	    srctmp = srcp;
	    srctmp = srctmp + src_len - 3;
	    if ((strncmp((char *)srcp,"(DNLM)",6) == 0) &&
	    ...
	    if (strncmp((char *)srcp,"(OCoLC)",7)==0) {
	      if(ocolc==0) {
		ocolc=1;
		if((auth999a)&&(ocolccnt>1))
		  cm_error(CM_WARNING,"Multiple OCLC#s: %*.*s",
			   src_len,src_len,srcp);
...
/*===========================================================================*/
    ...
    /* Leader 07 not 's' or 'b', exclude $a=(DNLM)* and all $9's */
    /* Read successive 035's */
    for (occ=0;
	 marc_get_field(pp->inmp, 35, occ, &srcp, &src_len)==0;
	 occ++) {
	...
	/* Drop any $a's that start with "(DNLM)" */
	if (sf_code == 'a') {
	  if(strncmp((char *)srcp,"(DNLM)",6)==0) {
	    /* But only if they aren't followed by: */
	    /* 1) "KIE" or "PIP" or "HSTAR" or "HISTLINE" or "NASA" */
	    /* 2) a colon followed by numeric data                  */
	    /* If we get one of these, make sure it does go out...  */
	    dcnt=0;
	    
	    if((strncmp((char *)srcp,"(DNLM)KIE:",10)==0)&&(src_len>10)) {
	      tmpp=srcp+10;
	      dcnt=isdigit(*tmpp);
	    }
MRVMRVMRV	    
	    if((strncmp((char *)srcp,"(DNLM)PIP:",10)==0)&&(src_len>10)) {
	      tmpp=srcp+10;
	      dcnt=isdigit(*tmpp);
	    }
	    
	    if((strncmp((char *)srcp,"(DNLM)HSTAR:",12)==0)&&(src_len>12)) {
	      tmpp=srcp+12;
	      dcnt=isdigit(*tmpp);
	    }
	    
	    if((strncmp((char *)srcp,"(DNLM)NASA:",11)==0)&&(src_len>11)) {
	      tmpp=srcp+11;
	      dcnt=isdigit(*tmpp);
	    }
	    
	    if((strncmp((char *)srcp,"(DNLM)HISTLINE:",15)==0)&&(src_len>15)) {
	      tmpp=srcp+15;
	      dcnt=isdigit(*tmpp);
	    }
	  ...
	  /* Ignore any $a's that start with "(OCoLC)" */
	  // 12/12/17 Removed this step - OCoLC's go out as-is 
	  //	      if(strncmp((char *)srcp,"(OCoLC)",7)==0)
	  //		break;
	}
	/* Drop the $9's but first copy it to the ui buffer */
...
/*===========================================================================*/
	...
	for (socc=2; ; socc++) {
      ...
      else /* End of 035's (may have been none) */
	break;
	...
	if (add_field_and_indics(pp, 35) != 0 )
	  ...
	  for (socc=2; ; socc++) {
		...
		if(strncmp((char *)srcp,"(OCoLC)",7)==0) {
		  if(ocolc) {
		    if(auth999a)
		      cm_error(CM_CONTINUE,"multiple OCLC#s: %*.*s",
			       src_len,src_len,srcp);
		    break;
		  }
		  else {
		    ...
		    if((strncmp((char *)srctmp,"ocm",3)==0) ||
		       (strncmp((char *)srctmp,"ocn",3)==0)) {
		      srctmp += 3;
		      cmp_buf_write(pp,"035:-1$a:+",
				    (unsigned char *)"(OCoLC)", 7, 0);
		      cmp_buf_write(pp,"035:-1$a:-1",
				    (unsigned char *)srctmp,
				    srcp+src_len-srctmp, 1);
		      break;
		    }
		  }
		}
